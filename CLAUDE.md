...
**文档自相矛盾时，执行者必须指出来，不得自行挑一条照做。**
派活时（无论给人还是给 Agent）都要把这条写进要求里。原因：契约/设计文档一旦前后打架，执行者"自己选一条"会把矛盾藏进代码，
等到联调或验收时才以"两边理解不一致"的形式爆出来，而那时已经写了几百行 —— 排查成本远高于当场问一句。
已发生的实例：`docs/api/README.md` §5.2 的 `data.tokenType` 说明**同一句里**既写"固定 `Bearer`"又写"不要硬编码"，
而 §5.6 的示例自己写死了 `Bearer`；前端执行者按 §5.2 的措辞实现并**主动上报**，矛盾才没有沉进代码（见 `docs/agent-log/20260913-阶段1前端实施.md`）。
配套纪律：**指出矛盾不等于可以停工** —— 能按更忠实的一条先做就做，同时把矛盾与自己的选择一并上报。
