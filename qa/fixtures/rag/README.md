...
|---|---|---|---|
| `公司年报.pdf` | **阶段验收夹具**：一份中文、含财务数字的真实年报（用户提供） | 1,682,689 字节；解析出 263,910 字符 → 587 块（Tika 3.2.3）；上传耗时 **实测 131.9 秒**（2026-09-22 端到端**经 8088**：Tika 解析 0.96 s + 向量化 37 批 ≈3.6 s/批 + 入库总计 136.6 s）。⚠️ **耗时只记实测值、不作判据** —— 它随机器负载与 CPU 漂移（设计初版按 2.1 s/批 估成 80~90 秒，实测偏慢约 1.5×）；判据只有"必须 < 300 秒"那一条上界（与前端逐请求上传超时同源，见 TC-03 §3.2-2 ④） | **不入库**（`.gitignore` 末段）。自备一份中文、含财务数字的 PDF，放本目录、保持同名即可 |
