...
- **发现者**：**用户**（2026-09-17 —— 要求 README 告知克隆位置，并直接提出「建一个**去除 hard code** 的任务」）
- **定界**：AI 会话（把「去除 hard code」经全仓库排查后**收窄**为「降低路径耦合」，理由见「为什么不是『去硬编码』」）
- **状态**：待定（未修；(b) 已随 README 落地）
- **命名说明**：本文件**不带 `[AI发现]` 前缀** —— 那个前缀在本目录专指「AI 主动发现、用户没提的问题」，本条不符。
