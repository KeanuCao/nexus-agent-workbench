...
> ⇒ 本阶段落地后，`scripts/sh/check-env.sh` 的 pgvector 检查项应从"扩展可用"升级为"扩展已安装"硬判据。**这是 devops 的连带任务**，列在 §6.6。
...
**连带要做**：`docker-compose.yml` 的 `ollama-init` 模型清单、`scripts/sh/lib/probe.sh` 的期望模型名
（都属 **devops**）；`docs/api/README.md` §4.5 的模型就绪判据（属**契约**，本节同步改）。
**重灌**：换模型后**必须重传文档**（本轮不保存原文件 ⇒ 只能重传；验收夹具实测 **131.9 s** ——
★ 本节原写的"约 90 秒"是换模型前的估算，**已作废**，见 §5.1-4/§7.2 与 §12）。
⚠️ **冻结区提示**：db-patch `202609221100` 的注释里仍引用着这个"约 90 秒"的口径
（它写在**已应用**的补丁里）。**不要去改它** —— 引擎对已应用补丁比对 checksum，
内容一变即 `EXIT_CHECKSUM_MISMATCH`、**迁移终止、后端起不来**（`PatchCli` 实测）。历史快照保持原样。
...
4. **上传/问答必须逐请求覆盖超时，且"超时 ≠ 失败"。**
   实例默认 `timeout: 15_000`，而**实测的上传耗时是它的数十倍**：**端到端 131.9 s**
   （2026-09-22 走 8088 实测：Tika 解析 0.96 s + 向量化 37 批 **134.7 s（≈3.6 s/批）**）。
   ★ 设计初版按 2.1 s/批 估成"80~90 秒"（那是宿主直连 Ollama 的估算），**实测偏慢 1.5 倍** ——
   这个数随机器负载漂移，故**耗时只当参考记录值、不当硬判据**（口径与 TC-03 一致，§7.2）。
   前端若用默认值，就会在**后端其实已经入库成功**时判定失败 ⇒ 用户重传 ⇒ 列表里出现两份同名文档。
   故：`uploadDocument` 传 **`timeout: 300_000`**（对实测 131.9 s 有 **2.3× 余量**，够覆盖"机器更慢"）、
   `askKb` 传 `120_000`（问答约 3~10 s）；且超时/网络错误的提示要写清楚
   ——**"请求超时，服务端可能仍在处理，请刷新列表确认后再重试"**。
   ⚠️ **已知边界（§9 风险 14）**：`max-chunks-per-document` 若真顶到 3000 块，按实测速率需
   **约 11 分钟**，必然先撞前端 300 s 超时 ⇒ 大文件走的就是上面那条"超时 ≠ 失败"路径。
...
# 5.6 第 5.5 步**失败**时怎么办（2026-09-22 实测遇到：**镜像源故障**，不是本机配置）
#     。症状：BuildKit 报 `failed to fetch anonymous token ... lookup docker.m.daocloud.io ... i/o timeout`，
#       legacy builder 报 `failed to resolve reference ... TLS handshake timeout`（同一故障两种面目）；
#       取证：WSL 宿主侧 `getent hosts docker.m.daocloud.io` 同样 unresolved。
#     。★ 先判"这一步是不是**内容中立**"，再决定要不要死等 —— 判据（比"重建成功"更直接）：
#       ① 容器内那份 nginx.conf 与宿主源码**逐字节相同**：
#          docker compose exec -T nexus-frontend sha256sum /etc/nginx/conf.d/default.conf
#          sha256sum docker-compose/frontend/nginx.conf
#       ② 本项目的 dist 是**卷挂载、逐请求读取**（不烘进镜像）⇒ 前端产物更新不依赖重建镜像
#       两条都成立 ⇒ **该偏差不影响功能**（8088 通路照常），网络恢复后补跑即可（预期 cache 命中）；
#       在故障窗口内反复重试是纯浪费 —— 2026-09-22 那次连试 4 次，症状在 DNS 与 TLS 之间漂移。
...
| # | 任务 | 判据 |
| --- | --- | --- |
| 1 | `scripts/sh/check-env.sh` 的 pgvector 检查升级：由"扩展可用"（`pg_available_extensions` 有行）升为"**已安装**"（`installed_version IS NOT NULL`）硬判据 | `docs/api/README.md` §4.6 已预告该升级时机；改后跑一次，应 PASS |
...
| 14 | **大文件的耗时必然超过前端 300 s 超时**（2026-09-22 实测后新识别） | 用户以为上传失败、重复上传，或以为"改了没生效" | 算式：`max-chunks-per-document` **3000** 块 ÷ 16 块/批 = 188 批 × 实测 **3.6 s/批** ≈ **11 分钟** `>` 前端 300s。**本轮接受并明说**：走"**超时 ≠ 失败**"路径（§5.1-4 的文案 + 刷新列表可见，服务端继续入库）。备选（未实施，记此备查）：① 上传改异步 + 轮询（要加任务表与进度接口）；② 前端超时提到 600s（仍不够大文件，只是往后挪）；③ 给"文件大小 ↔ 预计耗时"一个前端提示。**演示口径**：验收夹具 1.6MB / 131.9 s，在 300s 内有 2.3× 余量 |
...
| 2026-09-22 | ★ **换 embedding 模型：`nomic-embed-text`(768) → `bge-m3`(1024)** | 起因：**端到端验收有一条判据不成立** —— 问「去年利润是多少」返回 `grounded=true` 但答"未找到"，引用的 5 条与问题无关。devops 用数字排除了四个替代假设（前缀/生成/解析/阈值），定位到**召回不足**：含答案的块在 587 块中**排第 27 名**，而无关块也拿 0.71~0.75 分（阈值标定救不了）。处置见新增的 **§0.3**：改 `EMBED_MODEL`/`EXPECTED_DIMENSION` 两个常量 + 前缀置空（`search_*:` 是 nomic 的模型卡建议，bge-m3 不期望）+ **新增补丁 `202609221100` 重建两表为 `vector(1024)`（清空数据）** + **重灌文档**。连带同步：`docker-compose.yml` 的 `ollama-init` 模型清单、`scripts/sh/lib/probe.sh`、契约 §4.5 的模型就绪判据（**均属"活配置"**，历史记录只标注不改写）。**仍未解决**：混合检索 / rerank / 分块粒度（§10.3，下一次复测若仍不进前 3 就按此顺序上）。另新增 §9 风险 13 记录本事件 |
| 2026-09-22 | ★ **415 修复的实机复验通过 + 耗时口径订正（4 处）** | devops 按合并后的 `19709d7` 跑完部署链路，逐条取证而非认 exit code：git-sync 前后 SHA 对照（`657f8f1 → 19709d7`）✓、jar 内抽出 `GlobalExceptionHandler.class` 命中 `multipart 接口收到非 multipart 请求` ✓、**8088 上浏览器实际拿到的 JS** 里拦截器原文可读（`e.data instanceof FormData && e.headers.delete("Content-Type")`）✓、后端重启后 `/api/health` 三项 UP ✓、`db-patch-migrate` 4 个补丁 checksum 一致跳过（`t_db_patch` 未变）✓。**两条判据**：A `POST /api/kb/documents` + `application/json` → **200 + 40001**，且日志同步出现 `multipart 接口收到非 multipart 请求（缺 Content-Type 或带错了头）：contentType=application/json`；B（阶段2 回归）`/api/chat/stream` + `text/plain` → **415 + 40002** ✓。**8088 真上传夹具**：1.68MB → **200 / 131.9 s**（`documentId=2`、`chunkCount=587`、`charCount=263910`，DB 侧 `min_idx=0 / max_idx=586` 与预期吻合）。**订正**：① §5.1-4 / §7.2 / §9 风险 9 的"**80~90 秒**"（按 2.1 s/批 估）→ **实测 131.9 s（≈3.6 s/批）**，并定案「**耗时只当参考记录值、不当硬判据**」（TC-03 同步改，上界与 300s 前端超时同源）；② §6.5 补 **5.6 步**：前端镜像重建遇**镜像源故障**（`docker.m.daocloud.io` DNS/TLS 超时，WSL 宿主侧同样 unresolved）时，**先判这一步是不是内容中立**（容器内 `nginx.conf` 与宿主源码 sha256 相同 + dist 是卷挂载）⇒ 两条成立即不影响功能，网络恢复后补跑，别在故障窗口反复重试；③ §9 新增**风险 14**：`max-chunks=3000` 按实测速率约需 **11 分钟** `>` 前端 300s ⇒ 大文件走"超时 ≠ 失败"路径（本轮接受并明说）。⚠️ 库内现有唯一文档即此次上传入库的 `documentId=2`（走查与提问可直接用它） |
