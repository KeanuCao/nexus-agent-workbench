...
| ➕ 0.1 **镜像不自包含的失败形态**（刻意行为，非缺陷） | 清空 `build-artifacts` 卷里的 jar 后 `up -d nexus-backend` → 容器**启动失败**并打印三步修复命令；后端缺失 jar 是"启动即崩"，而前端是 nginx 照常起来、**全部 404 + 容器 unhealthy** —— 两者形态不同，别混淆 |
