...
> | §4.3 九步流程 | 第 2/4/5/6 步 | ❌ 已过期：新九步见 `scripts/up.sh` 头部注释 |
...
>
> **代价与新约束**（已确认接受）：
> ① 裸 `docker compose up -d` 不再是可靠入口，必须走 `scripts/up.sh` 的顺序；
...
> **`bge-m3`**；唯一真源是 `docker-compose.yml` 的 `ollama-init` 命令与 `scripts/lib/probe.sh` 的默认清单。
...
| 🔄 0.3 up.sh 一键拉起全套 | 判据从「完成 **§4.3** 全流程」改为「完成 **`scripts/up.sh` 头部注释的九步**」—— §4.3 是重构前的九步（旧第 4/5/6 步是"迁移 / 构建镜像 / 拉起容器"，新链路拆成"同步源码 → 迁移 → 打包 → 建镜像并拉起"四步）。干净环境一条命令跑通、最终健康检查 UP |
