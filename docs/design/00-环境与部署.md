...
> ## ⚠️ 2026-09-13 修订：builder 架构已变更（读本文档前必看）
>
> `CLAUDE.md` 的「构建容器（builder）」条目被改写，builder 从**多阶段构建的 stage 1 / 运行期不常驻**
> 改为 **手工启停的常驻容器 + 容器内 git pull + 产物落共享目录**。受影响的章节：
>
> | 章节 | 原内容 | 现状 |
> |---|---|---|
> | §2.1 拓扑表 | builder「不常驻，按需 `run --rm`」 | ❌ 已过期：改为手工 `up -d` / `stop` 的常驻容器 |
> | §2.1 拓扑表 | 前后端「本地多阶段构建」 | ❌ 已过期：运行镜像只剩 JRE / nginx，**产物从共享卷读取** |
> | §2.4 打包容器 | 多阶段构建（共用 stage 1） | ❌ 整节已被取代，见下方修订块与 `docs/design/01-多租户与认证.md` §2 |
> | §3.1 / §3.2 | PatchCli「由 builder 容器经 **mvn exec:java** 执行」 | ❌ 已过期：`exec:java` 与 `System.exit` 交互会导致**退出码不可靠**，而 up.sh 完全依赖退出码。现由 `db-patch-migrate` 脚本用 `java -cp` 直跑 |
> | §4.3 九步流程 | 第 2/4/5/6 步 | ❌ 已过期：新九步见 `scripts/sh/up.sh` 头部注释 |
> | §7 验收对照表 | 「多阶段构建」「runtime 不含工具链」相关判据 | 🔄 **已逐行核对并修订**：「判据本身仍成立、只是理由变了」这个初判**是错的** —— 有 3 行（builder 共用构建阶段 / up.sh 完成 §4.3 / 补丁"再次执行必须报错"）**判据本身已失效**，另补了 4 行新架构验收点 |
>
> **以下章节保留原文**（作为 0.1/0.4 当年的决策留档，未删除），阅读时以上表为准。
...
Windows 11 宿主机（开发机）
 ...
 ├─ 一键触发：wsl -d nexus-agent-workbench -- bash /mnt/c/wp/nexus-agent-workbench/scripts/sh/up.sh
...
> **⚠️ 2026-09-13 修订：本节描述的多阶段构建方案已被取代，保留原文仅作决策留档。**
>
> 新架构（依据改写后的 `CLAUDE.md`）：
>
> ```
> builder 容器（手工启停，不挂载宿主源码）
>   挂载：builder-src → /workspace（容器内 git 工作区）
>         builder-m2 / builder-npm → 依赖缓存
>         build-artifacts → /artifacts（★ 产物共享目录）
>   入口：git-sync / build-backend / build-frontend / build-all / db-patch-migrate
>         （都在镜像内 /usr/local/bin，手工与 up.sh 走同一批入口）
>                     │
>                     ▼ 命名卷 build-artifacts
>    nexus-backend  ── 只读挂载 → java -jar /artifacts/backend/app.jar
>    nexus-frontend ── 只读挂载 → nginx root /artifacts/frontend
> ```
>
> **关键差别**：产物不再 COPY 进镜像层，而是落在共享卷里被前后端**挂载消费**。
> 因此运行镜像**不再自包含** —— 产物没生成时后端容器会启动失败（入口脚本给了可读报错）。
>
> **代价与新约束**（已确认接受）：
> ① 裸 `docker compose up -d` 不再是可靠入口，必须走 `scripts/sh/up.sh` 的顺序；
> ② **必须先 push** —— 容器内 `git pull` 只能拿到远端已有的提交，宿主未推送的改动构建时看不到。
>
> 详细落地设计见 `docs/design/01-多租户与认证.md` §2 与 `docker-compose/builder/Dockerfile` 头部注释。
...
### 2.5 Ollama 模型拉取策略（qwen2.5:7b、bge-m3）

> ⚠️ **2026-09-22 晚：向量化模型已由 `nomic-embed-text` 换为 `bge-m3`**（起因：中文检索召回不足，
> 见 `docs/design/03-RAG知识库.md` §0.3）。本节下面提到 `nomic-embed-text` 的地方，期望模型名一律读作
> **`bge-m3`**；唯一真源是 `docker-compose.yml` 的 `ollama-init` 命令与 `scripts/sh/lib/probe.sh` 的默认清单。
...
- 引擎位置：`nexus-infrastructure` 模块（`com.nexus.infrastructure.dbpatch.PatchCli`，Java main 类，**零第三方编译期依赖**：只 import `java.*`）；**由 builder 容器执行**，**不在后端启动流程中执行**。执行方式见 §3.2。
  > **2026-09-13 修订**：原文写"经 mvn 执行（`mvn -pl nexus-infrastructure exec:java`）"—— 该写法已弃用，原因是 `exec:java` 与 `System.exit` 的交互会让**退出码不可靠**，而 `up.sh` 完全依赖退出码决定"后端是否拉起"。现改为 `docker-compose/builder/db-patch-migrate` 脚本先用 Maven 编译并解析 classpath，再 `java -cp … PatchCli` 直跑，`exec` 原样透传退出码。
  > 补丁目录的来源也随之变化：不再是"随仓库源码进入 builder 构建上下文"，而是**容器内 git 工作区里的 `/workspace/repo/db-patch`** —— 宿主目录仍是开发真源，但**必须先 push**，容器才拉得到（详见 §2.4 修订块）。
...
sequenceDiagram
    ...
    BU->>CLI: 脚本 java -cp 直跑 PatchCli（nexus-net 网络内；非 mvn exec，见 §3.1 修订）
    CLI->>DB: 连接可用性探测
...
| 文件名不匹配命名规则 | **只对 `.sql` 文件成立**：扫描阶段先按后缀过滤（`endsWith(".sql")`），**非 `.sql` 文件静默忽略**（不计入「扫描到 N 个 .sql 文件」、不告警、不影响退出码）；以 `.sql` 结尾但名字不合规者 → 警告并跳过。<br>「目录允许放说明性文件」这句落在这里 —— 放 `README.md` 是安全的（被静默忽略），但**不会**有警告。<br>⚠️ **2026-09-17 措辞收口**：原句「文件名不匹配命名规则 → 警告并跳过」按字面读会把 `README.md` 也算进去，与实现不符（TC-00-0.2-6 实测为证：注入 `README.md` **观察不到** WARN）。实现见 `PatchCli.java:387` 的扫描过滤 + `PatchCli.java:135-140` 的命名检查 |
...
- `scripts/sh/check-env.sh`、`scripts/sh/up.sh` 存于仓库（Windows 侧可见），**只在 WSL `nexus-agent-workbench` 内执行**。
- Windows 一键触发（写入 README）：`wsl -d nexus-agent-workbench -- bash -c "cd /mnt/c/wp/nexus-agent-workbench && ./scripts/sh/up.sh"`；可选提供根目录 `up.cmd` 薄包装（§8 待确认）。
...
```
1. bash scripts/sh/check-env.sh            → 任一 FAIL 中止
2. docker compose build builder   → 先构建专门打包容器（git+mvn+npm+psql）
   （注意：**服务名是 `builder`，`nexus-builder` 是镜像名**。
     builder 虽挂在 `profiles: ["build"]` 下，但**显式点名服务即会激活其 profile**
     —— 2026-09-11 实测 `docker compose build builder` 与
     `docker compose --profile build build builder` 均可，前者不会报 `no such service`。
     但**必须显式点名服务**：不带服务名时 targets 为 nexus-backend + nexus-frontend，
     会把尚未就绪的应用镜像一起拉进构建。
     另注：`build nexus-frontend` 也会连带构建 nexus-backend（`depends_on` 生效），
     而 `build nexus-backend` 只有它自己一个 target。）
...
#### 版本号单一真源（2026-09-15 修订）

- **唯一真源 = `backend/pom.xml` 的 `<revision>`**（该文件 `<properties>` 段内）：后端自身版本号
  （= 全部 6 个模块的版本）只此一处，发版只改这一行（**已实测**：0.2.1 → 0.2.2 就是一行，
  见 `docs/test-cases/TC-00.md` 的 TC-00-0.1-7）。取真源值：`grep -o '<revision>[^<]*</revision>' backend/pom.xml`。
- **子模块 `<parent><version>` 一律写 `${revision}`，禁止写死具体版本号** ——
  这样它请求的 parent GAV 与 `../pom.xml` 必然字符串相等，从**结构上**消除版本漂移（而不是靠"记得同步改"）。
- 父 POM 的 `<version>` 同样是 `${revision}`；install/deploy 前由 **flatten-maven-plugin** 把它落成字面量
  （`flattenMode=resolveCiFriendliesOnly`、`updatePomFile=true`），否则装进本地仓库的 POM 里仍是
  `${revision}` 字面量，IDE 单模块导入 / 只 clone 子目录的消费者解析不了。
  配置位置与逐项理由见 `backend/pom.xml` 文件头「版本策略」与 `<build>` 段注释。
- 运行时观测点：`application.yml` 的 `@project.version@` 经资源过滤注入后，
  `GET /api/health` 的 `data.version` 与 `/actuator/info` 的 `info.app.version` 都应等于该 `<revision>`。
- **历史故障（2026-09-15 实测，勿重演）**：父 POM `0.2.1`、子模块 `<parent><version>` 写死 `0.2.0` 时，
  Maven 在 `../pom.xml` 读到 GAV 不匹配（0.2.1 ≠ 被请求的 0.2.0），转而**从本地仓库**解析
  `nexus-parent:0.2.0`；又因 `builder-m2` 是持久卷、里面正好存着旧版本，**构建成功且零警告**，
  产物版本仍是旧的（现场日志：`Building nexus-common 0.2.0` 与 `Building nexus-parent 0.2.1` 并存）。
  只有把本地仓库里的 `com/nexus` 挪走，才会响亮失败：
  `Non-resolvable parent POM for com.nexus:nexus-common:0.2.0 ... 'parent.relativePath' points at wrong local POM`。
  **教训**：写死版本号 + 持久化本地仓库 = 静默用旧版本构建，比直接报错更危险 —— 报错至少会让人停下来。
- **判据（两条都要，缺一条就可能漏掉上面的静默漂移）**：
  ① 构建日志里**不得**出现 `relativePath` 字样（出现即说明子模块没继承到父 POM 的版本、正在回落本地仓库）；
  ② `cd backend && mvn -B validate` 输出的 `Building nexus-*` **六行版本必须一致**（且等于 `<revision>`）。
...
| Node | 20 LTS（**builder 镜像内经 nodesource 源补装**，非独立 `node:20-alpine` 基础镜像） | **（2026-09-11 修订）** 原文写"与 Windows 侧本地开发用同一大版本，避免行为漂移"——实测 Windows 侧为 **Node 24**，该意图已不成立。实际是靠**依赖锁定的 engines 兼容范围**兜住的（vite 5.4.21 的 engines 为 `^18.0.0 \|\| >=20.0.0`，rollup/plugin-vue 同理），**不是靠版本对齐**。排查构建问题时不要往"版本要对齐"这条使劲。附带：Node 20 的官方维护期已于 2026-04-30 结束（演示项目不阻塞，升级时只需改 `builder/Dockerfile` 一处） |
...
> ⚠️ 样例里的 `"version": "0.2.0"` 是**示例值，非真源；实际以 `/api/health` 的实际返回为准**
> （真源与判据见 §5.1「版本号单一真源」与 `docs/api/README.md` §4.2.2）。
...
> **⚠️ 2026-09-13 修订**：本表已按 builder 重构后的架构**逐行核对并更新**。
> 上一版把本节标注为「判据本身仍成立，但成立理由变了」——**该判断是错的**：
> 至少「builder 共用构建阶段」与「up.sh 完成 §4.3 全流程」两行，判据本身也已失效（详见下方标 🔄 的行）。
> 标注：✅ 未变 ｜ 🔄 已修订 ｜ ➕ 新增（新架构带来的验收点）。
...
| 🔄 0.1 后端前端独立容器；单独的专门打包容器 builder 构建前后端包、**运行期不常驻**（共用构建阶段） | 旧判据（`docker compose ps` 无 builder；前后端镜像**以 nexus-builder 为 stage 1**）**已失效** —— 两个 Dockerfile 现在都没有 stage 1。新判据：① builder 是**手工启停的常驻容器**（`docker compose up -d builder` 可起、`stop builder` 可停，且裸 `up -d` 不拉起它）；② 前后端运行镜像只含 JRE / nginx，`docker history nexus-backend` **无 mvn 层**；③ 产物来自共享卷而非镜像层（见下两行） |
| ➕ 0.1 **容器内 git pull 源码**（不挂载宿主源码） | `docker compose exec builder git-sync` → 日志打印的提交号与 `origin/<NEXUS_REPO_BRANCH>` 一致；容器内 `/workspace/repo` 存在且宿主 `backend/` **未**被挂载进去 |
| ➕ 0.1 **产物落共享目录** | `docker compose exec builder build-all` → `docker run --rm -v build-artifacts:/a alpine ls -lR /a` 能看到 `backend/app.jar` 与 `frontend/index.html`；前后端容器**只读挂载**该卷 |
| ➕ 0.1 **换包即生效**（不必重建镜像） | 重跑 `build-backend` 后 `docker compose restart nexus-backend` → `curl localhost:8089/api/health` 的 `data.version` 变为新版本号 |
| ➕ 0.1 **必须先 push**（新架构最硬的工作流约束） | 宿主 commit 但**不 push** → 容器内 `git-sync` 后构建出的仍是旧提交；表现是"我明明改了怎么没生效" |
| ➕ 0.1 **镜像不自包含的失败形态**（刻意行为，非缺陷） | 清空 `build-artifacts` 卷里的 jar 后**`restart nexus-backend`**（⚠️ 不是 `up -d` —— 对运行中的容器那是空操作，实测 `StartedAt` 不变）→ 容器**启动失败**（`Restarting (1)` 循环）并打印三步修复命令。<br>前端**不是「全部 404」**（2026-09-15 实测订正，原判据是推演）：产物缺失且后端在跑时，`/` → **403**、`/index.html` 及其他路径 → **404**（`try_files` 末尾的 `=404` 是同日补的，补之前是 500 —— 回退目标不存在时 nginx 内部重定向死循环）、容器 **unhealthy**。<br>⚠️ **级联（已知行为，刻意不修）**：若此时后端不在运行，nginx 会在启动期 `[emerg] host not found in upstream "nexus-backend"` 而**启动失败**，前端跟着进 `Restarting` 循环 —— 保留它是为了让「前端依赖后端」以最响亮的方式暴露 |
| 🔄 0.2 未执行补丁按文件名排序依次应用 | **判据改造（2026-09-17）**：原文「**清库重跑迁移**」已**废除** —— 删业务表即使"能靠重跑补丁恢复"也不算无害（纪律见 `.claude/agents/qa-engineer/CLAUDE.md` 的《Test Harmlessness》）。新口径：在**一次性沙箱库**上从零重放真实补丁目录（`NEXUS_PG_DB=nexus_tc00_probe` + `NEXUS_PATCH_DIR=/workspace/repo/db-patch`），断言 `t_db_patch.file_name` 顺序与目录字典序一致、种子数据到位；**真实 `nexus` 库全程零写操作**。可执行步骤见 `docs/test-cases/TC-00.md` §2 |
| 🔄 0.2 已执行补丁再次执行报错终止 | **语义已订正**（§3.3）：checksum **一致 → 静默跳过（退出码 0）**；只有**篡改**（checksum 不一致）才终止。判据：① 重跑迁移 → 全跳过、退出码 0、行数不变；② 篡改已应用补丁 → 退出码**非 0** 并打印新旧 checksum。<br>**实测口径（2026-09-17）**：② 的篡改对象改为 `qa/fixtures/db-patch/` 的**探针补丁**（与业务无关），不再动业务补丁文件 —— 见 `docs/test-cases/TC-00.md` §2 |
| ✅ 0.2 变更走新增补丁 | 评审约定 + checksum 机制兜底 |
| ✅ 0.3 check-env.sh 覆盖 Docker/WSL/端口/镜像源/模型 | 逐项触发 PASS/WARN/FAIL 各路径 |
| 🔄 0.3 up.sh 一键拉起全套 | 判据从「完成 **§4.3** 全流程」改为「完成 **`scripts/sh/up.sh` 头部注释的九步**」—— §4.3 是重构前的九步（旧第 4/5/6 步是"迁移 / 构建镜像 / 拉起容器"，新链路拆成"同步源码 → 迁移 → 打包 → 建镜像并拉起"四步）。干净环境一条命令跑通、最终健康检查 UP |
| ✅ 0.3 wsl.abc.md 记录排查经验 | 文件存在且遵循四段式记录法，随实践持续追加 |
| ✅ 0.4 `mvn clean install -DskipTests` 通过 | CI/本地执行退出码 0（**注**：容器内构建已改为 `mvn clean install`，见 §2.4 —— 版本号变更会让新旧 jar 并存，不带 `clean` 会打破"恰好一个 `*.jar`"的约定） |
...
**阶段总验收**：拿到仓库 → 执行 up.sh 一键拉起 → 后端健康检查、前端页面均可达。

> **TC-00 的展开依据就是本表**（2026-09-13 起）：上表已逐行核对，可直接展开为可执行用例。
> 用例格式以 [`docs/test-cases/TC-01.md`](../test-cases/TC-01.md) 为模板（分段表 + 「实测」栏留空）。
> ⚠️ **两份 TC 对「测不出来」的处理方式不同，是有意为之**：TC-00 集中在文末附表，TC-01 则分散写进相关用例的备注与头部说明
> （2026-09-18 用户决定取消 TC-01 的附表）。**不是漏了，也不是失效** —— 两边都要求「测不出来的项必须交代去向」。
