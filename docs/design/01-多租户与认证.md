...
`scripts/up.sh` 第 4 步是三段式条件执行，已交付、已评审：
...
| # | 验收标准（原文） | 验证方式 |
| --- | --- | --- |
| 0.2-1 | 未执行补丁按文件名排序依次应用 | **清空 `t_db_patch` 与业务表后重跑迁移**，比对 `t_db_patch.file_name` 顺序与目录字典序 |
| 0.2-2 | 已执行补丁再次执行报错终止（checksum + 记录表幂等保护） | ① 重跑迁移 → 全部跳过、退出码 0；② **篡改任一已应用补丁一个字符** → 迁移**报错终止（退出码非 0）**并打印新旧 checksum |
