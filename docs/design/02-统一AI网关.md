...
| # | 决策 | 备选 | 理由 / trade-off |
| --- | --- | --- | --- |
| **D1** | **HTTP 客户端用 Spring 6.1 `RestClient`**；**流式读取必须走 `execute(requestCallback, responseExtractor)` 的 extractor 内部读取**，不用 `exchange()` 返回后再读 | ① OkHttp ② JDK `HttpClient` ③ WebClient | 项目已有先例（`OllamaProbe` 类注释写明「不引入 OkHttp/WebClient 等重量级依赖」），一个项目两套 HTTP 客户端是坏味道。**为什么必须是 extractor 形式**：`RestClient` 的响应体转换会经 `HttpMessageConverterExtractor`，而 `RestTemplate.doExecute` 家族惯常在调用返回时关闭 `ClientHttpResponse`（`SimpleClientHttpResponse.close()` 还可能 drain 流）—— 那样拿到的是**已失效的流**，问题不是"增量与否"而是"流还在不在"。在 extractor 内部读则无论 Spring 关不关都安全。**代价**：写法比 OkHttp 啰嗦。**待实测确认**：JDK `HttpURLConnection` 是否真增量交付分片 —— 判据见 §8 的 2.3-1；退路是换 JDK `HttpClient`（同样零依赖） |
...
2. **`fetch` 不走 axios 拦截器 → 有两个头要自己补，不是一个。**
   ...
   - 另外 `fetch` **不对非 2xx 抛错**，必须自己判 `response.ok`；判断响应类型要用 `includes('application/json')`（Spring 会给 `text/event-stream` 带 `;charset=UTF-8`，全等比较会误判）。
...
> **为什么用 `env_file` 而不是 `${DEEPSEEK_API_KEY:-}` 插值**：插值走"宿主 shell 环境 + `docker-compose/.env`"两个来源，而 `.env` 是**提交进仓库**的（`.gitignore` 第 6 行的例外），密钥绝不能放；放 shell 则要求每次开新终端都 `export`，而 `scripts/up.sh` 是通过 `wsl -d ... -- bash -c` 调起的**非交互 shell，不读 `~/.bashrc`** —— 那个 `export` 很可能压根不生效。`env_file` 直接读文件，与调用方式无关。
...
> **待实测确认**：`required: false` 这个长语法需要 Compose ≥ 2.24。**好消息是失败很响亮**：`check-env.sh` 检查点 6 跑 `docker compose config --quiet`，语法不被支持时**直接 FAIL**，而 `up.sh` 第 1 步就是它 → 整条链路中止，不会静默。**验证方式**：跑一次 `./scripts/check-env.sh`，看检查点 2 打印的 `docker compose version --short`（该检查点本来就会打印），检查点 6 会连带验证 `env_file` 长语法。
...
**待实测确认**：改完需复跑 `/api/health` 与 `scripts/check-health.sh`，确认前端静态页与其余接口无回归（§8 最后一行）。`check-health.sh` 第 6 段只做功能探活（状态码 + body 判 `"code"`），`check-env.sh` 不解析 `nginx.conf` —— 所以**复跑 `./scripts/check-health.sh` 就是齐全的回归判据**。
...
| 3 | **RestClient 不真增量交付 / 响应流已被关闭** | 2.3-1 不过 | D1 已把实现形状定成 extractor 形式（规避 close 风险）；"是否真增量"**待实测**，退路是换 JDK `HttpClient` |
