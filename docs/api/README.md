...
## 4. 健康检查判据（`scripts/sh/check-env.sh` / `scripts/sh/check-health.sh` 的唯一判据来源）
...
✅ 好消息：`scripts/sh/check-health.sh:379` 本来就是按"**不断言时区**"实现的（注释里写着"容器内 Z=UTC、
...
> `docker-compose.yml` 的 `ollama-init` 命令与 `scripts/sh/lib/probe.sh` 的默认清单（两处由 devops 同步）。
...
3. **超时 ≠ 失败**：上传在**请求线程**上同步完成（数十秒~两分钟量级，取决于文件大小与 CPU），
   前端必须逐请求覆盖超时（**上传 300s**、问答 120s —— 验收夹具 587 块上传**实测 131.9 s**
   （2026-09-22 经 8088 端到端实测，两次一致：129.75 s / 131.9 s；向量化 37 批 ≈ **3.6 s/批**），
   原定的 120s 太贴边 —— 实测值已越过它；⚠️ **耗时随机器负载漂移，只当参考记录值**，
   判据只有 **< 300 s** 这一条上界；`timeout` 的取值以设计 §5.1-4 为准），且超时文案要写成
