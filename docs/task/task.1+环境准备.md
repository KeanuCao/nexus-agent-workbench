...
- **验收标准**：
  - 判据常量**只在本文件出现一次**：期望模型名（`qwen2.5:7b`、`nomic-embed-text`）、单次超时（10s / 5s）、端口读取方式
...
- [ ] 0.2 db-patch 补丁工作流 —— 🚧 **代码已交付（2026-09-13），待实机验收**
  ...
  - **未验证**（勾选前必须补）：① 首次迁移实际执行成功、`t_db_patch` 落 2 行；② 幂等复跑全跳过且退出码 0；
    ③ **篡改已应用补丁 → 退出码非 0 并打印新旧 checksum**（2026-09-17 起：篡改对象用 `qa/fixtures/db-patch/` 的探针补丁，不碰业务补丁）；④ 乱序保护（时间戳回退 → 非 0）；
    ⑤ 排序（**一次性沙箱库**从零重放真实补丁目录，比对 `file_name` 与目录字典序；原「清库重跑」口径已废 —— 见 `docs/design/00-环境与部署.md` §7）；⑥ up.sh 第 5 步端到端
