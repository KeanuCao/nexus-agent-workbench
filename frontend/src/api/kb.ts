 ...
 * 解析 → 分块 → 向量化 → 入库，实测夹具 `公司年报.pdf`（587 块、37 批 **≈3.6 s/批**，bge-m3 跑
 * CPU、无 GPU）**端到端 131.9 s**（2026-09-22 经 8088 实测；同日更早一次 129.75 s，**两次一致**）。
 * ⇒ `300_000` 对实测值有 **≈2.3× 余量** —— **这才是这个数字的意义**（不是"卡着估算值放"）。
 * ★ 本注释早先写的"37 批 × 约 2.1s ⇒ 80~90 秒"**已作废**：那是宿主直连 Ollama 的早期估算，
 * 实测偏低 1.5~2 倍。设计 §5.1-4 / §7.2 已定案：**耗时只当参考记录值、不当硬判据** ——
 * 它随机器负载与 CPU 漂移，拿它当预期值只会得到"假失败"。
