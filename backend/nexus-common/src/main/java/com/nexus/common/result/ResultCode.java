...
public enum ResultCode {
     ...
     *
     * <p><b>判在向量化之前</b>（决策 D15）：10MB 的 TXT ≈ 2 万块 ÷ 每批 16 块 = <b>1250 批</b>，
     * 按实测速率 ≈ <b>3.6 s/批</b>（bge-m3 / CPU / 无 GPU，2026-09-22 端到端实测）要跑
     * <b>≈ 75 分钟</b>（1250 × 3.6 s ≈ 4500 s）；而"先分块、数一眼、再决定调不调 embedding"
     * 让它在<b>几秒内</b>以本码失败 —— 分块一结束就判，此时还没进事务、一次 embedding 都没调。
     * 不判的话，用户看到的是一次像上游故障的超时（{@link #CHAT_UPSTREAM_UNAVAILABLE}），
     * 排查方向整个是错的。
     *
     * <p>⚠️ <b>75 分钟是"没有这道闸"的反事实，不是耗时预期</b>：超限文件不会进向量化，
     * 那正是本码存在的全部意义。耗时口径一律只当参考记录值、不当判据（设计 §5.1-4）。
     * <b>代码里</b>这道闸的算式只写在本段（其余提到它的地方只留结论 + 指回这里）：
     * 初版按 2.1 s/批 估的"80~90 秒"与"几分钟后"都是宿主直连 Ollama 时的估算，**均已作废**。
     *
